...
def analyze_llm_numeric(prompt, verbose=False):
    """
    Calculates probabilities for numbers 0-100 and generates the full text response.
    Returns: Expected Value (weighted avg), Entropy, Response Text
    """
    messages = [
        {"role": "system",
            "content": "You are a professional stock analyst. Predict the probability of the stock going up. Answer with only an integer between 0 and 100 and do not say anything else."},
...
def run_trial(columns, verbose=False):
    ...
    for index, row in df.iterrows():
        ...
        prompt_text = f"The stock {joined_context}."
...
if __name__ == "__main__":
    ...
    # Control
    control_prompt = "The stock is unknown."
    ...
    # Save
    print("\nSaving numeric_out.json...")
    with open("data/numeric_out.json", "w") as json_file:
        json.dump(out, json_file, indent=4)

    print("Saving detailed_numeric_results.csv...")
    results_df = pd.DataFrame(detailed_logs)
    results_df.to_csv("data/detailed_numeric_results.csv", index=False)
    print("Done.")
